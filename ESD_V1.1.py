...
class ExcelSQLApp:
    ...
    def setup_query_panel(self, frame):
        ...
        buttons = [
            ("▶ Execute", self.execute_query_handler),  # Single execute button
    ...
    def execute_query_handler(self):
        ...
        Executes selected text if available, otherwise executes the entire editor content.
        """
        query_to_execute = ""
        try:
            # Check if there's any selected text
            if self.query_text.tag_ranges(tk.SEL):
                query_to_execute = self.query_text.get(tk.SEL_FIRST, tk.SEL_LAST).strip()
            else:
                query_to_execute = self.query_text.get("1.0", tk.END).strip()
        except tk.TclError:
            # This can happen if SEL tag is not properly set, fallback to full text
            query_to_execute = self.query_text.get("1.0", tk.END).strip()

        self._execute_core_query(query_to_execute)
